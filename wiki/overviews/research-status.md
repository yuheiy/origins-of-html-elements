...
表モデル7要素、フォーム基本部品4要素、文書・組版語彙5要素、WHATWG内部派生4要素の一括調査を完了した。最後の群では、`audio`をゲーム等のWebアプリケーションの効果音、`progress`をWeb上の非アクセシブルな画像製進捗バー、`source`を単一コーデックへ統一できないメディア環境、`summary`を追加情報を要約するGUI開閉ラベルまで遡った。特定実装や同名語彙からの採用記録がない箇所は接続していない。残る`br`、`head`、`link`、`menu`、`small`、`style`は既に指定資料または変更区間を調査しているため、新しい具体的資料が見つかった場合だけ再開する。
...
前節のHTML外への到達度で抽出した26要素のうち、20要素の一括再調査を完了した。残る6要素は指定資料をすでに探索済みであり、新しい具体的資料が見つかった場合だけ再開するため、現在ただちに反復する優先対象はない。
