...
表モデル7要素に加え、フォーム基本部品の`input`、`select`、`option`、`button`は調査を完了した。フォーム群では初期Web検索とProtek Pilot等の業務用フォーム経験を要素の背景へ接続し、特定GUIツールキットからの一対一の移植は否定した。`button`の初出は1997年から1996年3月へ訂正した。残る調査は、文書・組版語彙の`abbr`、`q`、`sub`、`sup`、`div`、WHATWG内部派生の`audio`、`progress`、`source`、`summary`の順にまとめて行う。`br`、`head`、`link`、`menu`、`small`、`style`は既に指定資料または変更区間を調査しているため、各群の探索で新しい具体的資料が見つかった場合に再開する。
...
現在の優先調査対象は、前節のHTML外への到達度で列挙した26要素から表モデル7要素とフォーム基本部品4要素を除く15要素である。公開された変更履歴または名前の分かる外部候補へ探索を限定できる群から順に調査し、一般的な類似探索は行わない。
