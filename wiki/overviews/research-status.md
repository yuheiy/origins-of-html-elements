...
現時点で優先して再調査するのは、HTML+で止まる`br`、`caption`、`input`、`select`、`option`、`q`、`sub`、`sup`、`table`、`td`、`th`、`tr`、IETF／W3C草案で止まる`abbr`、`button`、`col`、`colgroup`、`div`、`head`、`link`、`small`、`style`、WHATWG内部派生で止まる`audio`、`progress`、`source`、`summary`、初期系統と再利用後の系統を分ける必要がある`menu`の26要素である。これらは外部材料への言及があっても、個別の採用因果または内発設計の根拠が不足しているため、確度Aや完成状態だけで由来確認済みとはしない。

表モデルの`table`、`caption`、`tr`、`th`、`td`、`col`、`colgroup`は、技術出版の要求を受けたHTML+内発設計と、後から部分的に入ったCALS交換要件を分離して調査を完了した。残る調査は、フォーム基本部品の`input`、`select`、`option`、`button`、文書・組版語彙の`abbr`、`q`、`sub`、`sup`、`div`、WHATWG内部派生の`audio`、`progress`、`source`、`summary`の順にまとめて行う。`br`、`head`、`link`、`menu`、`small`、`style`は既に指定資料または変更区間を調査しているため、各群の探索で新しい具体的資料が見つかった場合に再開する。
...
現在の優先調査対象は、前節のHTML外への到達度で列挙した26要素から表モデル7要素を除く19要素である。公開された変更履歴または名前の分かる外部候補へ探索を限定できる群から順に調査し、一般的な類似探索は行わない。
