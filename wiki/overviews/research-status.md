...
確度と状態はHTMLへの導入経路の品質を評価するため、HTML外の由来へ到達したかを別に判定する。外部の文書体系、ソフトウェア、メディア、UI慣習から要素への採用または翻案を直接確認できる場合を`外部系譜確認済み`、特定の外部祖先はないがHTML外の要求に対する新設計だったことを同時代資料で確認できる場合を`内発設計確認済み`、候補または一般的背景だけがある場合を`外部候補のみ`、HTML標準化内部までしか遡れていない場合を`外部未確認`とする。この分類は調査優先度を定める概観上の評価であり、要素ページの確度や状態を置き換えない。

この監査で優先対象として抽出したのは、HTML+で止まっていた`br`、`caption`、`input`、`select`、`option`、`q`、`sub`、`sup`、`table`、`td`、`th`、`tr`、IETF／W3C草案で止まっていた`abbr`、`button`、`col`、`colgroup`、`div`、`head`、`link`、`small`、`style`、WHATWG内部派生で止まる`audio`、`progress`、`source`、`summary`、初期系統と再利用後の系統を分ける必要がある`menu`の26要素である。これらは抽出時点で外部材料への言及があっても、個別の採用因果または内発設計の根拠が不足していたため、確度Aや完成状態だけで由来確認済みとはしなかった。

表モデル7要素、フォーム基本部品4要素に加え、文書・組版語彙の`abbr`、`q`、`sub`、`sup`、`div`は調査を完了した。`sub`／`sup`はLaTeX・TeXを直接の設計材料へ接続し、`q`は言語別の引用符慣習からRFC 2070、HTML 4への決議までを接続した。`abbr`は自然言語上の略語カテゴリを求める同時代コメントとの一致まで、`div`はHTML WG内部の競合案と合意までに限定し、未確認の採択因果や外部DTDを接続しなかった。残る一括調査は、WHATWG内部派生の`audio`、`progress`、`source`、`summary`である。`br`、`head`、`link`、`menu`、`small`、`style`は既に指定資料または変更区間を調査しているため、新しい具体的資料が見つかった場合だけ再開する。
...
現在の優先調査対象は、前節のHTML外への到達度で列挙した26要素から完了した16要素を除く10要素である。うち`audio`、`progress`、`source`、`summary`を次の一括調査対象とし、残る6要素は指定資料をすでに探索しているため、新しい具体的資料が見つかった場合だけ再開する。
