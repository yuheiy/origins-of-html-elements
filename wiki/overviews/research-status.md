...
現行115要素の導入確度はAが87要素、A−が15要素、Bが12要素、Cが1要素で、未評価はない。ただしAはHTMLへの導入と直前の系譜だけを評価するため、Aであっても語の命名理由やHTML外の由来が確定したとは限らない。
...
導入確度とは別に、HTML外の由来へ到達した結果を`外部系譜確認済み`、`外部要求確認済み`、`外部候補あり`、`外部未確認`、`未評価`へ分類する。判定基準は[AGENTS.md](../../AGENTS.md)を正本とし、個別要素の分類は移行後の要素ページを正本とする。移行中はこの概観から分類を機械的に転記せず、各ページの証拠と未解決点を再確認する。

この監査で優先対象として抽出したのは、HTML+で止まっていた`br`、`caption`、`input`、`select`、`option`、`q`、`sub`、`sup`、`table`、`td`、`th`、`tr`、IETF／W3C草案で止まっていた`abbr`、`button`、`col`、`colgroup`、`div`、`head`、`link`、`small`、`style`、WHATWG内部派生で止まる`audio`、`progress`、`source`、`summary`、初期系統と再利用後の系統を分ける必要がある`menu`の26要素である。これらは抽出時点で外部材料への言及があっても、個別の採用因果または外部要求に応じた設計の根拠が不足していたため、導入確度Aや完成状態だけで由来確認済みとはしなかった。
