...
- [要素](elements/)は、現行要素ごとに現在の意味、HTMLへの導入、直前の祖先、HTML外の由来と到達度、証拠、導入確度、否定された仮説、未解決点をまとめる。既存ページは新しいHTML外への到達度へ順次移行する。
- [共有系譜](lineages/)は、複数要素に共通する確認済みの採用経路や要求をまとめる。個別要素の結論、導入確度、HTML外への到達度は要素ページを正本とする。
