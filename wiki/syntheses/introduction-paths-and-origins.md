...
現行115要素の確度はAが86要素、A−が16要素、Bが12要素、Cが1要素で、未評価はない。ただしAはHTMLへの導入と直前の系譜だけを評価するため、Aであっても語の命名理由やさらに上流の由来が確定したとは限らない。
...
集合単位の採用しか確認できない、または中間経路が欠けるA−は、`dd`、`del`、`dl`、`dt`、`footer`、`head`、`ins`、`li`、`link`、`nav`、`ol`、`output`、`q`、`style`、`u`、`ul`の16要素である。個別対応を集合レベルの資料から推定してはならない。
...
`HTML直前の祖先`に未確認部分を明記するのは、`a`、`address`、`aside`、`audio`、`base`、`blockquote`、`br`、`canvas`、`col`、`del`、`div`、`html`、`ins`、`menu`、`noscript`、`output`、`p`、`pre`、`progress`、`q`、`s`、`section`、`small`、`source`、`style`、`sub`、`sup`、`title`、`u`の29要素である。背景資料があるだけで、これらの欠落へ矢印を補ってはならない。
...
| 調査群 | 対象 | 探索範囲 |
|---|---|---|
| W3C／IETF草案間の変更 | [`style`](../elements/style.md) | 連続する草案間に限定して、CVS／RCS差分、Working Group議事録、編集者メールから追加、改名、機能統合、後続仕様への採録を探す |
