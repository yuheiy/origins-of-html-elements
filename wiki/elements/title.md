...
> 状態: 完成（敵対的レビュー済み: 2026-08-09）。1991年の出現と設計目的を検証済み（直前祖先は未確認）
...
敵対的レビュー（2026-08-09）では、Waterlooの同名`TITLE`、1991年当事者mail、CERNタグ一覧、1993年draftを再照合した。先行GMLとの名称一致はHTMLへの採用因果を示さないため系譜へ結ばず、HTML側の設計目的だけを確認したBを維持した。
