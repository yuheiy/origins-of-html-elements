...
表のcolumn summariesを含むfooter rowsをまとめる要素である。1995年7月のIETF table草案がCALS table modelにならったhead/body/foot row groupingとして提案し、RFC 1942を経てHTML 4.0が標準化した。
...
1995年草案とRFC 1942はCALSより前のfoot row sectionの祖先を特定していない。
...
[集合単位] CALS table modelのhead/body/foot row grouping → IETF table草案 `TFOOT`（1995年） → RFC 1942 `TFOOT` → HTML 4.0 `TFOOT` → 現行HTML `<tfoot>`
...
1995年草案がCALSから`TFOOT`を含むrow-group modelへの因果関係を直接明記する一方、記述は三要素の集合単位であり、CALS側の個別要素との対応を確認できないため。
...
- 1995年7月草案より前に`TFOOT`を加えた議論または編集履歴。
...
敵対的レビュー中の反証探索（2026-08-09）で、RFC 1942より早い`draft-ietf-html-tables-00`の`TFOOT`定義とCALS由来文を確認した。初出と系譜を訂正し、集合レベルの制約は維持した。
