...
1993年HTML draftがTexinfoのマクロ名由来と明記した論理的な文字強調要素で、同時代Texinfoの`@strong`との対応を確認できる。
...
1993年6月のHTML Internet-Draftに埋め込まれたHTML DTDはRCS識別子`html.dtd,v 1.3 93/01/06`を持ち、`inline` entityに`STRONG`を列挙するため、formal DTDへの収録を1993年1月6日まで遡って確認できる。同draft本文は文字強調elementとして`<strong>`を列挙し、その集合の要素名をTexinfoのマクロ名から派生したと明記する。このRCS日付は実装初出や、それ以前のprototypeがなかったことを示すものではない。[1993年HTML Internet-Draft](https://datatracker.ietf.org/doc/html/draft-ietf-iiir-html-00)
