...
`meter`は2006年、WHATWG Web Applications 1.0のWIP `gauge`を改名し、既知範囲のスカラー量の測定を表す要素として仕様化された。
...
WHATWG Web Applications 1.0の`gauge`要素。Git差分が同じ節の改名と、既知範囲の測定という意味の継承を直接示す。

## さらに上流の由来

### 証拠

初回Git収録の`gauge`はディスク利用状況とクエリー結果の関連性を例示し、比率、割当量の利用状況、得票率、良好／不適切な範囲の表現を検討していた。

### 解釈

一般的な計器の外見ではなく、値の範囲と望ましい領域を機械可読に表す要求が`gauge`から`meter`へ引き継がれた。

## 系譜

WHATWG Web Applications 1.0 `gauge` → 2006年リビジョン15 `meter` → HTML5 `meter` → 現行HTML `meter`
...
特定OSのメーターウィジェット、CSS進捗バー、または計器一般から直接採用されたとは確認できないため、外見と名称だけで由来へ結ばない。
