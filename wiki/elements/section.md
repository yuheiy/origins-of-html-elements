...
遅くとも2004年8月にはWHATWG Web Applications 1.0で定義され、見出しだけでは文書構造とアウトラインを確実に復元できない問題へのセクショニングモデルを担った。2004年11月に編集者は、既存の`div class="section"`を`section`へ置換できることを設計要求として明記した。

## 現在の意味

WHATWG HTML Living Standardでは、文書またはアプリケーション内の汎用的なセクション、すなわち通常は見出しを伴う主題別の内容群を表す。スタイル指定やスクリプティングだけの汎用コンテナーではない。[HTML Living Standard](../../raw/html.spec.whatwg.org/multipage/sections.html)（[公開版](https://html.spec.whatwg.org/multipage/sections.html#the-section-element)、2026-08-10確認）
...
2004年のメールで、既存の見出しを保ったまま`div class="section"`を専用要素へ置換し、アウトラインを構成する要求を確認できる。2005年草案で、見出しと組み合わせて文書構造を表すセクショニングモデルを確認できる。

### 解釈

既存Webの汎用コンテナーに埋め込まれていたセクション構造を、旧UAでの見出し表示を保ちながら明示する要素として導入されたと解釈できる。XHTML 2等の別語彙からの影響は確定しない。
...
既存Webの`div class="section"`を`section`へ置換可能にするという要素単位の設計要求を、WHATWG編集者の同時代メールで直接確認できるため。
