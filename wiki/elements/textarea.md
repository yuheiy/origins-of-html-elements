...
長い複数行値を`INPUT`属性へ置くSGML上の問題からHTML+で分離され、HTML 2.0へ採録された要素である。

## 現在の意味

WHATWG HTML Living Standardでは、生の値を編集する複数行のプレーンテキスト入力欄を表し、要素内容はコントロールの既定値を表す。[HTML Living Standard](../../raw/html.spec.whatwg.org/multipage/form-elements.html)（[公開版](https://html.spec.whatwg.org/multipage/form-elements.html#the-textarea-element)、2026-08-09確認）
...
複数行テキストUIとの対応はあるが、要素を分離した直接理由は史料が述べるSGML属性長問題である。
...
|---|---|---|---|---|
| 1993-11-08 | [HTML+ §10](../../raw/www.w3.org/MarkUp/HTMLPlus/htmlplus_41.html)（[公開元](https://www.w3.org/MarkUp/HTMLPlus/htmlplus_41.html)） | Internet-Draft | `TEXTAREA`のHTML+のフォーム上の設計を確認できる。 | 2026-08-09 |
...
特定GUIツールキットの複数行テキストウィジェットをそのまま移植したという説明。直接史料がない。
