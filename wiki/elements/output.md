...
`output`はXFormsの非編集可能な値表示コントロールを含む機能集合を既存HTMLフォームモデルへ適応する過程で、2003年にHTMLフォームコントロールとして提案され、Web Forms 2.0を経てHTML5へ統合された。
...
XFormsの`output`を含む機能集合。2003年案はXForms機能をHTMLフォームへ加える方針を明記し、同名の非編集可能な値表示コントロールをHTML DOMとスクリプト中心のモデルへ作り直した。ただし`output`単独の採用記録は未確認である。
...
HTML版はXFormsのインスタンスデータ結び付けをそのまま移植せず、動的な表示内容をデータモデルではなく内容として扱い、DOM値をスクリプトで更新する既存HTMLフォームに近いコントロールへ作り直した。

## 系譜

XForms `output`を含む機能集合 → 2003年HTMLフォーム案`output` → Web Forms 2.0 `output` → HTML5 `output` → 現行HTML `output`
...
|---|---|---|---|---|
| 2000-12-19 | [*XForms 1.0: XForms User Interface*](../../raw/www.w3.org/TR/2000/WD-xforms-20001219/ui.html)（[公開元](https://www.w3.org/TR/2000/WD-xforms-20001219/ui.html#N1052)） | W3C Working Draft | 非編集可能なデータ値表示用`output`フォームコントロール | 2026-08-10 |
...
XForms機能をHTMLフォームへ加える因果と同名コントロールの対応は直接確認できるが、集合単位の説明しかなく`output`単独の採用判断が欠ける。

## 否定された仮説

HTML `samp`や一般的なプログラム出力から生まれたとは確認できないため、名称や現在の用途だけで`SAMP → OUTPUT`を結ばない。

## 未解決

- 2003年9月案から12月4日案までのどの議論で`output`追加を決め、XForms `output`との対応を選んだか。
- XForms結び付けを外す理由以外に、`value`、`defaultValue`、リセットというHTML固有のDOMモデルを選んだ要素単位の判断記録はあるか。
