...
function outputFor(source) {
  ...
  if (!pathname || pathname.endsWith("/")) pathname += "__index";
...
function archiveKind(buffer, contentType) {
  const signature = buffer.subarray(0, 6).toString("hex");
  if (["504b0304", "504b0506", "504b0708"].some((prefix) => signature.startsWith(prefix)) || contentType === "application/zip") return "zip";
  if (["1f8b", "1f9d", "425a68", "fd377a585a00", "28b52ffd"].some((prefix) => signature.startsWith(prefix)) || buffer.subarray(257, 262).toString() === "ustar" || ["application/x-tar", "application/gzip", "application/x-gzip", "application/x-compress", "application/x-bzip2", "application/x-xz", "application/zstd"].includes(contentType)) return "tar";
  return "file";
}
...
const kind = archiveKind(body, response.headers.get("content-type")?.split(";", 1)[0].trim().toLowerCase());
if (kind !== "file") {
